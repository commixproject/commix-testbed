...
$__tail = <<<'TESTBED_TAIL'
    ...
    <aside class="panel">
      ...
      <p class="explain">The body is JSON read from <code>php://input</code> and its <code>addr</code> member goes into <code>exec()</code>. The payload has to sit inside the JSON rather than be appended to it.</p>
...
<title>JSON request body &middot; commix-testbed</title>
...
<div class="wrap page">
  ...
  <div class="cols">
    <section class="panel live">
      <h2>The page</h2>
      <div class="howto"><p>This page takes no form input - it reads the raw request body. Send it something shaped like this:</p><pre><code>POST /scenarios/regular/POST/classic_json.php
Content-Type: application/json

{"addr":"127.0.0.1","name":"ancst"}</code></pre><p class="note">Both members are read; only <code>addr</code> reaches the command.</p></div><b><?php
                // JSON request format:
                // {"addr":"127.0.0.1","name":"ancst"}
                // read JSon input
                $data_back = json_decode(file_get_contents('php://input'));
